...
from py_rl.py_server.simple_scheduler import Scheduler


class Logger:

    def __init__(self):
        self.task_stat_scalars_cpu_usage = {}
        self.task_stat_scalars_mem_usage = {}
        self.init_task_stat_scalars()

    def init_task_stat_scalars(self):
        for ns in Scheduler.ns_list:
            app_dict_cpu = {}
            app_dict_mem = {}
            for app in Scheduler.apps_dict[ns]:
                app_dict_cpu.setdefault(app, 0)
                app_dict_mem.setdefault(app, 0)
            self.task_stat_scalars_cpu_usage.setdefault(ns, app_dict_cpu)
            self.task_stat_scalars_mem_usage.setdefault(ns, app_dict_mem)

    def get_taint_str(self, taints):
        taint_str = ""
    ...
    def get_node_string(self, prefix, request):
        node_str = "{}\tuuid: {}\n" \
                "{}\tfriendly_name: {}\n" \
                "{}\tresource_type: {}\n" \
                "{}\tstate: {}\n" \
                ...
                "\n".format(prefix, request.resource_desc.uuid,
                           ...
                           prefix, self.get_label_str(request.resource_desc.labels),
                           prefix, self.get_taint_str(request.resource_desc.taints),
                           prefix,
        ...
        if len(request.children) > 0:
            for i, child in enumerate(request.children):
                node_str += "{}+++++Child {}+++++\n".format(prefix, i)
                prefix += "\t"
                node_str += self.get_node_string(prefix, child)
        return node_str


    def log_added_node(self, request):
        log_str = "=====NODE ADDED=====\n"
        log_str += self.get_node_string("", request)
        log_str += "\n"
        logging.debug(log_str)


    def log_updated_node(self, request):
    ...
    def get_toleration_str(self, tolerations):
        tolerations_str = ""
    ...
    def get_node_affinity_str(self, node_affinity):
    ...
    def get_pod_affinity_term_str(self, pod_affinity_term):
    ...
    def get_pod_affinity_str(self, pod_affinity):
        ...
        for i, pod_affinity_term in enumerate(pod_affinity.requiredDuringSchedulingIgnoredDuringExecution):
            ...
            pod_affinity_str += self.get_pod_affinity_term_str(pod_affinity_term)
        ...
        for i, weighted_pod_affinity_term in enumerate(pod_affinity.preferredDuringSchedulingIgnoredDuringExecution):
            ...
            pod_affinity_str += self.get_pod_affinity_term_str(weighted_pod_affinity_term.podAffinityTerm)


    def get_task_desc_str(self, task_descriptor):
        task_str = "======Task Descriptor======\n" \
            ...
            .format(task_descriptor.uid,
                    ...
                    self.get_label_str(task_descriptor.labels),
                    self.get_toleration_str(task_descriptor.toleration),
                    self.get_node_affinity_str(task_descriptor.affinity.node_affinity),
                    self.get_pod_affinity_str(task_descriptor.affinity.pod_affinity),
                    self.get_pod_affinity_str(task_descriptor.affinity.pod_anti_affinity)
                    )

        return task_str


    def get_job_desc_str(self, job_descriptor):
    ...
    def log_submitted_task(self, request):
        log_str = "=====TASK ADDED=====\n"
        log_str += self.get_task_desc_str(request.task_descriptor)
        log_str += self.get_job_desc_str(request.job_descriptor)
        log_str += "\n"
        logging.debug(log_str)


    def log_task_stats(self, task_stat):
        log_str = "=====TASK STATS=====\n" \
                  "\ttask_id: {}\n" \
                  "\thostname: {}\n" \
                  "\ttimestamp: {}\n" \
    ...
    def log_node_stats(self, resource_stat):
        cpus_stats_str = ""
    ...
    def log_add_task_info(self, request):
    ...
    # new brief loggers
    def brief_log_task_completed(self, pod_uid):
    ...
    def brief_log_task_failed(self, pod_uid):
        log_str = "=====Task Failed=====\n" \
                  "\t pod_uid: {}\n" \
                  "\n" \
                  .format(pod_uid)
    ...
    def brief_log_task_removed(self, pod_uid):
    ...
    def brief_log_task_submitted(self, raw_pod):
        ...
        log_str = "=====Task Submitted=====\n" \
                  ...
                  "\t app_name: {}\n" \
                  "\t pod_vec: {} \n" \
                  "".format(raw_pod.uid, raw_pod.raw_namespace, raw_pod.raw_pure_name, raw_pod.vec)
        logging.info(log_str)

    def brief_log_node_added(self, raw_node):
        log_str = "=====Node Added=====\n" \
                  "\t node_uuid: {}\n" \
                  "\t node_name: {}\n" \
                  "\t node_vec: {}\n" \
    ...
    def brief_log_task_stats(self, raw_pod, writer, time, epoch):
        ns = raw_pod.raw_namespace
        app_name = raw_pod.raw_pure_name
        cpu_usage = raw_pod.task_stat_cpu_usage
        mem_usage = raw_pod.task_stat_mem_usage
        ...
        log_str = "=====Task Stats=====\n" \
                  ...
                  "\t cpu_usage: {}MilliCore \n" \
                  "\t mem_usage: {}Kb \n" \
                  "".format(raw_pod.uid, ns, app_name, cpu_usage, mem_usage)
        logging.info(log_str)
        self.task_stat_scalars_cpu_usage[ns][app_name] = cpu_usage
        writer.add_scalars("cpu_usage/{}".format(ns), self.task_stat_scalars_cpu_usage[ns], epoch)
        self.task_stat_scalars_mem_usage[ns][app_name] = mem_usage
        writer.add_scalars("mem_usage/{}".format(ns), self.task_stat_scalars_mem_usage[ns], epoch)
        epoch += 1

    def brief_log_node_stats(self, raw_node):
        log_str = "=====Node Stats=====\n" \
                  "\t node_uuid: {}\n" \
                  "\t node_name: {}\n" \
                  "\t node_vec: {}\n" \
